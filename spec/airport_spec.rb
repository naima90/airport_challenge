...
describe Airport do
  ...
  it 'prevent take-off when weather is stormy' do
    ...
    expect {subject.take_off(:plane) }.to raise_error 'can not take off as weather is stormy'
  end

  it 'prevent landing when weather is stormy' do
    allow(subject).to receive(:stormy?).and_return true
    expect {subject.land(:plane) }.to raise_error 'can not land as weather is stormy'
  end
end